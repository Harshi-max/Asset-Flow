...
export default function DashboardPage() {
  return (
    <div className="space-y-8 pb-12">
          ...
          {quickLinks.map((link) => {
            ...
            return (
              <Link
                key={link.href}
                href={link.href}
                className={`group flex flex-col items-center gap-2.5 rounded-2xl border ${link.border} bg-white p-4 text-center shadow-sm transition hover:-translate-y-0.5 hover:shadow-md dark:bg-slate-900`}
              >
                ...
                <div>
                  ...
                  <p className="text-[10px] leading-tight text-slate-400 dark:text-slate-500">{link.description}</p>
                </div>
                <ArrowRight className="h-3 w-3 text-slate-300 transition group-hover:translate-x-0.5 group-hover:text-indigo-500 dark:text-slate-600" />
              </Link>
            );
          })}
        </div>
      ...
      {/* Full Executive Dashboard with KPIs, Charts, Activity Feed, AI Assistant */}
      <ExecutiveDashboard />
    </div>
  );
}
